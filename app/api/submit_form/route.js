...
export async function POST(req, res) {
    ...
    try {
        ...
        if (
            !sanitizedUsername || !sanitizedStartDate
        ) {
            ...
            !usernameValidator(sanitizedUsername) || !dateValidator(sanitizedStartDate)
        ) {
        ...
        const userExists = await db.get('SELECT username FROM user WHERE username = ?', [sanitizedUsername], (err, row) => {
        ...
        db.run(
            query,
            [sanitizedUsername, sanitizedStartDate, sanitizedCurrentStreak, sanitizedMotivationalMessage, started_at, updated_at],
            function (error) {
